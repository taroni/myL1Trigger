...
_eleID                            (iConfig.getParameter<std::string>("eleID"))
{
  ...
  // Exceptions...
  if(_mode!="ML" && _mode!="HZZ" && _mode!="Fake" && _mode!="EP" && _mode!="TP" && _mode!="TP_nadir") 
    throw cms::Exception("Configuration") << "Parameter 'mode' must be either 'ML','HZZ','Fake','EP', or 'TP', or 'TP_nadir' \n";
...
SkimAllPathsFilter::~SkimAllPathsFilter()
{
  ...
  cout << " N 2 Lepton TP nadir-skimmed event = " << n_ele2TP_skimmed_nadir << endl;
  ...
  // Efficiency
  double eff_1lepton = 0; double eff_2lepton = 0; double eff_2eleTP =0 ; double eff_2eleTP_nadir =0 ; double eff_3lepton = 0; 
  ...
  if(n_init!=0) { 
    ...
    eff_2eleTP_nadir = double(n_ele2TP_skimmed_nadir)/double(n_init);
  ...
  cout << " Eff 2 leptonTP nadir skim   = " << eff_2eleTP_nadir << endl;
...
bool SkimAllPathsFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  int nEle_et20  = 0; int nEleId_et20 = 0; int nEleIdNoIso_et20 = 0; 
  int nEle_et15  = 0; int nEleId_et15 = 0;
  int nEle_et10  = 0; int nEleId_et10 = 0;
  int nEle_et5   = 0; int nEleId_et5  = 0;

  int nEleId_sclEt5 = 0; // for E/P skim
  ...
  for(unsigned int eleIt = 0; eleIt < EleHandle->size(); ++eleIt)
    {      
      ...
      if (!eleRef->ecalDrivenSeed() && eleRef->trackerDrivenSeed()) sclRef = eleRef->pflowSuperCluster();

      
      

      
      double R  = TMath::Sqrt(sclPos.x()*sclPos.x() + sclPos.y()*sclPos.y() +sclPos.z()*sclPos.z());
      double Rt = TMath::Sqrt(sclPos.x()*sclPos.x() + sclPos.y()*sclPos.y());
      ...
      double rel_isotrack = eleRef->dr03TkSumPt()/eleRef->pt();
      double rel_isohcal  = (eleRef->dr03HcalDepth1TowerSumEt() + eleRef->dr03HcalDepth2TowerSumEt() )/eleRef->pt();
      double rel_isoecal  = eleRef->dr03EcalRecHitSumEt()/eleRef->pt();
      ...
      if( eleRef->pt() > 20) {cout<<"test"<<endl; ++nEle_pt20; if(eleIDBool && eleIsoBool) nEleId_pt20++; if(eleIDBool) nEleIdNoIso_pt20++;}

      if( sclET > 5)  { ++nEle_et5;  if(eleIDBool && eleIsoBool) nEleId_et5++;  }
      if( sclET > 10) { ++nEle_et10; if(eleIDBool && eleIsoBool) nEleId_et10++; }
      if( sclET > 15) { ++nEle_et15; if(eleIDBool && eleIsoBool) nEleId_et15++; }
      if( sclET > 20) { ++nEle_et20; if(eleIDBool && eleIsoBool) nEleId_et20++; if(eleIDBool) nEleIdNoIso_et20++;}
      
      
      if( sclET>5. && eleIDBool && eleIsoETBool) nEleId_sclEt5++;
      if(  eleRef->pt()>5. && eleIDBool && eleIsoETBool) nEleId_sclEt5++;
  ...
  // default asking in cfg.py file

  // 2 Eleskim T&P custom Nadir
     if ( (nEle_et5 >=2 && nEleId_et5>=1) ) { ele2TP_skim_nadir = true ; n_ele2TP_skimmed_nadir++;} 
  ...
  // E/p Skim
  if( nEleId_sclEt5 > 1) { ep_skim = true; n_ep_skimmed++;}

  // Select...
  if(_mode == "ML")   { if(old_lepton1_skim==true || lepton2_skim==true || lepton3_skim==true) return true; }
  if(_mode == "HZZ")  { if(lepton3_skim==true)cout<<"test"<<endl;  return true; }
  if(_mode == "Fake") { if(fakerate_skim==true) return true; }
  if(_mode == "EP")   { if(ep_skim==true)       return true; }
  if(_mode == "TP")   { if(ele2TP_skim==true)       return true; } // default mode in cfg.py file
  if(_mode == "TP_nadir")   { if(ele2TP_skim_nadir==true)       return true; } // custom
