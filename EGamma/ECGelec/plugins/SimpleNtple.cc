...
// ====================================================================================
void SimpleNtple::FillTrigger (const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
	...
	if(!aod_) {
	  ...
	  for( l1extra::L1EmParticleCollection::const_iterator emItr = emIsolColl->begin(); emItr != emIsolColl->end() ;++emItr) {
	    // Used by Clemy
	    _trig_L1emIso_ieta[isocounter] = emItr->gctEmCand()->regionId().ieta();
	    _trig_L1emIso_iphi[isocounter] = emItr->gctEmCand()->regionId().iphi();
	    _trig_L1emIso_rank[isocounter] = emItr->gctEmCand()->rank(); // ET in ADC count... 1 ADC count = 0.5 GeV
	    // From Trigger twiki
	  ...
	  for( l1extra::L1EmParticleCollection::const_iterator emItr = emNonisolColl->begin(); emItr != emNonisolColl->end() ;++emItr){  
	    // Used by Clemy
	    _trig_L1emNonIso_ieta[nonisocounter] = emItr->gctEmCand()->regionId().ieta();
	    _trig_L1emNonIso_iphi[nonisocounter] = emItr->gctEmCand()->regionId().iphi();
	    _trig_L1emNonIso_rank[nonisocounter] = emItr->gctEmCand()->rank(); // ET in ADC count... 1 ADC count = 0.5 GeV
	    // From Trigger twiki
	    _trig_L1emNonIso_eta[nonisocounter]    = emItr->eta();
	    _trig_L1emNonIso_phi[nonisocounter]    = emItr->phi();
	    _trig_L1emNonIso_energy[nonisocounter] = emItr->energy();
	    _trig_L1emNonIso_et[nonisocounter]     = emItr->et();
	    nonisocounter++;
	  ...
	  for(ipsbel=psbel.begin(); ipsbel!=psbel.end(); ipsbel++) {
	    ...
	    if(rank>0) {
	      ...
	      // Used by Clemy
	      _trig_preL1emNonIso_ieta[nonisocounter] = regionEtaRec;
	      _trig_preL1emNonIso_iphi[nonisocounter] = int(((*ipsbel)>>10)&0x1f);
	      _trig_preL1emNonIso_rank[nonisocounter] = rank;
	      nonisocounter++;
	    }
	  }//loop Noniso
	  _trig_preL1emNonIso_N = nonisocounter;
	  ...
	  for(ipsbel=psbel.begin(); ipsbel!=psbel.end(); ipsbel++) {
	    ...
	    if(rank>0) {
	      ...
	      // Used by Clemy
	      _trig_preL1emIso_ieta[isocounter] = regionEtaRec;
	      _trig_preL1emIso_iphi[isocounter] = int(((*ipsbel)>>10)&0x1f);
	      _trig_preL1emIso_rank[isocounter] = rank;
	      isocounter++;
	    }
	  }//loop Iso
	  _trig_preL1emIso_N = isocounter;
	  ...
	  for(ipsbel2=psbel2.begin(); ipsbel2!=psbel2.end(); ipsbel2++) {
	    ...
	    if(rank>0) {
	      ...
	      // Used by Clemy
	      _trig_postL1emNonIso_ieta[nonisocounter] = regionEtaRec;
	      _trig_postL1emNonIso_iphi[nonisocounter] = int(((*ipsbel2)>>10)&0x1f);
	      _trig_postL1emNonIso_rank[nonisocounter] = rank;
	      nonisocounter++;
	    }
	  }//loop Noniso
	  _trig_postL1emNonIso_N = nonisocounter;
	  ...
	  for(ipsbel2=psbel2.begin(); ipsbel2!=psbel2.end(); ipsbel2++) {
	    ...
	    if(rank>0) {
	      ...
	      // Used by Clemy
	      _trig_postL1emIso_ieta[isocounter] = regionEtaRec;
	      _trig_postL1emIso_iphi[isocounter] = int(((*ipsbel2)>>10)&0x1f);
	      _trig_postL1emIso_rank[isocounter] = rank;
	      isocounter++;
	    }
	  }//loop Iso
	  _trig_postL1emIso_N = isocounter;
...
// ====================================================================================
void SimpleNtple::FillMuons(const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
  // Beam spot
...
// ====================================================================================
void SimpleNtple::FillJets(const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
...
// ====================================================================================
void SimpleNtple::FillSuperClusters(const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
...
// ====================================================================================
void SimpleNtple::FillTruth(const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
...
// ====================================================================================
void  SimpleNtple::FillTipLipIp(const edm::Event& iEvent, const edm::EventSetup& iSetup)
// ====================================================================================
{
  //Get the B-field
  edm::ESHandle<MagneticField> magneticField;
...
// ====================================================================================
void SimpleNtple::beginJob(const edm::ParameterSet& conf)
// ====================================================================================
{
...
// ====================================================================================
bool SimpleNtple::IsConv (const reco::GsfElectron & eleRef) //edm::Ref<reco::GsfElectronCollection> eleRef)
// ====================================================================================
{
...
// ====================================================================================
const EcalRecHit SimpleNtple::getRecHit(DetId id, const EcalRecHitCollection *recHits)
// ====================================================================================
{
	if ( id == DetId(0) ) {
		return EcalRecHit();
...
//GETTING RCT regions
// ====================================================================================
int SimpleNtple::getGCTRegionPhi(int ttphi)
// ====================================================================================
{
	int gctphi=0;
	gctphi = (ttphi+1)/4;
...
// ====================================================================================
int SimpleNtple::getGCTRegionEta(int tteta)
// ====================================================================================
{
	int gcteta = 0;
...
// ====================================================================================
float SimpleNtple::E2overE9( const DetId id, const EcalRecHitCollection & recHits, 
...
// ====================================================================================
float SimpleNtple::recHitE( const DetId id, const EcalRecHitCollection &recHits )
// ====================================================================================
{
        if ( id == DetId(0) ) {
                return 0;
...
// ====================================================================================
float SimpleNtple::recHitApproxEt( const DetId id, const EcalRecHitCollection &recHits )
// ====================================================================================
{
